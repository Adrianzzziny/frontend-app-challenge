...
export default function AddAccountModal({ isOpen, onClose, onAddAccount }: AddAccountModalProps) {
  ...
  const { control, handleSubmit, reset, formState: { errors, isValid } } = useForm<AddAccountFormValues>({
          ...
          <ScrollView className="flex-1 px-5 pt-4" showsVerticalScrollIndicator={false}>
            ...
            <View className="space-y-4">
              ...
              <Controller control={control} name="numeroCuenta" render={({ field: { onChange, value } }) => (
                <BaseInput label="Número de cuenta" placeholder="Escribe tu cuenta destino" value={value} onChangeText={onChange} error={errors.numeroCuenta?.message} keyboardType="numeric" maxLength={20} />
              )} />
